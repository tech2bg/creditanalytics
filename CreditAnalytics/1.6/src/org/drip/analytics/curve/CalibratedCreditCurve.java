...
public class CalibratedCreditCurve extends org.drip.analytics.definition.CreditCurve {
		...
		final org.drip.param.definition.NodeTweakParams ntp)
	{
		...
		try {
			return new CalibratedCreditCurve (_dblStartDate, _strName, adblHazardBumped, _adblHazardDate,
			...
			adblHazardDate.length || adblHazardRate.length != adblHazardDate.length || null ==
				adblRecoveryRate || 0 == adblRecoveryRate.length || null == adblRecoveryDate || 0 ==
					adblRecoveryDate.length || adblRecoveryRate.length != adblRecoveryDate.length ||
						java.lang.Double.isNaN (dblStart))
	...
	@Override public CalibratedCreditCurve createParallelHazardShiftedCurve (
		final double dblShift)
	{
		...
		try {
			return new CalibratedCreditCurve (_dblStartDate, _strName, adblHazard, _adblHazardDate,
				_adblRecoveryRate, _adblRecoveryDate, _dblSpecificDefaultDate);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
	...
	@Override public CalibratedCreditCurve createParallelShiftedCurve (
		final double dblShift)
	{
		if (java.lang.Double.isNaN (dblShift)) return null;
		...
		CalibratedCreditCurve cc = null;
		double[] adblCalibQuote = new double[_adblCalibQuote.length];

		org.drip.analytics.calibration.ComponentCalibrator calibrator = new
			org.drip.analytics.calibration.NewtonRaphsonCalibrator();

		try {
			cc = new CalibratedCreditCurve (_dblStartDate, _strName, _adblHazardRate, _adblHazardDate,
				_adblRecoveryRate, _adblRecoveryDate, _dblSpecificDefaultDate);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
	...
	{
		if (java.lang.Double.isNaN (dblFlatNodeValue) || 0. >= dblFlatNodeValue || null == _valParam ||
			null == _aCalibInst || 0 == _aCalibInst.length || null == _adblCalibQuote || 0 ==
		...
		try {
			if (bSingleNode)
				cc = org.drip.analytics.creator.CreditCurveBuilder.FromHazardNode (_dblStartDate, _strName,
					_adblHazardRate[0], _adblHazardDate[0], java.lang.Double.isNaN (dblRecovery) ?
						_adblRecoveryRate[0] : dblRecovery);
			else
				cc = new CalibratedCreditCurve (_dblStartDate, _strName, _adblHazardRate, _adblHazardDate,
					_adblRecoveryRate, _adblRecoveryDate, _dblSpecificDefaultDate);
		} catch (java.lang.Exception e) {
	...
	{
		if (java.lang.Double.isNaN (dblValue) || iNodeIndex > _adblHazardRate.length) return false;
		...
		final org.drip.param.definition.NodeTweakParams ntp)
	{
		...
		if (org.drip.param.definition.CreditNodeTweakParams.CREDIT_TWEAK_NODE_PARAM_RECOVERY.equalsIgnoreCase
			...
			try {
				return new CalibratedCreditCurve (_dblStartDate, _strName, _adblHazardRate, _adblHazardDate,
			...
			if (org.drip.param.definition.CreditNodeTweakParams.CREDIT_TWEAK_NODE_MEASURE_HAZARD.equalsIgnoreCase
				...
				try {
					return new CalibratedCreditCurve (_dblStartDate, _strName, adblHazardBumped,
				...
				try {
					if (cntp._bSingleNodeCalib)
						...
						cc = new CalibratedCreditCurve (_dblStartDate, _strName, _adblHazardRate,
							_adblHazardDate, _adblRecoveryRate, _adblRecoveryDate, _dblSpecificDefaultDate);
				} catch (java.lang.Exception e) {
					e.printStackTrace();
	...
	@Override public double getSurvival (
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblDate)) throw new java.lang.Exception ("No surv for NaN date");

		if (dblDate <= _dblStartDate) return 1.;

		if (!java.lang.Double.isNaN (_dblSpecificDefaultDate) && dblDate >= _dblSpecificDefaultDate)
			return 0.;
	...
	@Override public double getRecovery (
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblDate)) throw new java.lang.Exception ("No rec for NaN date");
	...
	@Override public org.drip.service.stream.Serializer deserialize (
		final byte[] ab) {
		try {
			return new CalibratedCreditCurve (ab);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
		...
		throws java.lang.Exception
	{
		...
		CalibratedCreditCurve cc = new CalibratedCreditCurve (dblStart, "XXS", adblHazardRate,
		...
		CalibratedCreditCurve ccDeser = new CalibratedCreditCurve (abCC);
