...
/**
 * This class contains the baseline discount curve holder object. It maintains term structure for the forward
 * 		rates, the calibration instruments, calibration measures, calibration quotes, and parameters.
 *
 * @author Lakshmi Krishnamurthy
 */

public class CalibratedDiscountCurve extends org.drip.analytics.definition.DiscountCurve {
	private static final int NUM_DF_QUADRATURES = 5;
	private static final boolean USE_NR_CALIBRATOR = true;

	private double _adblCDFRate[] = null;
	private double _adblCDFNode[] = null;
	...
	public CalibratedDiscountCurve (
		...
		throws java.lang.Exception
	{
		if (null == adblDate || 0 == adblDate.length || null == adblRate || adblDate.length !=
			...
			throw new java.lang.Exception ("Invalid inputs into the Discount Curve constructor");

		_strCurrency = strCurrency;
		_adblCDFNode = new double[adblDate.length];
		_adblCDFRate = new double[adblRate.length];

		for (int i = 0; i < _adblCDFNode.length; ++i)
			_adblCDFNode[i] = adblDate[i];

		for (int i = 0; i < _adblCDFRate.length; ++i)
			_adblCDFRate[i] = adblRate[i];

		_dblStartDate = dtStart.getJulian();
	}

	protected CalibratedDiscountCurve (
		final CalibratedDiscountCurve dc)
		throws java.lang.Exception
	{
		if (null == dc) throw new java.lang.Exception ("Invalid input Discount Curve");
		...
		_adblCDFRate = dc._adblCDFRate;
		_adblCDFNode = dc._adblCDFNode;
	 ...
	 * @param ab Byte Array
	 * 
	 * @throws java.lang.Exception Thrown if DiscountCurve cannot be properly de-serialized
	 */
		...
		throws java.lang.Exception
	{
		if (null == ab || 0 == ab.length)
			throw new java.lang.Exception ("DiscountCurve de-serializer: Invalid input Byte array");

		java.lang.String strRawString = new java.lang.String (ab);

		if (null == strRawString || strRawString.isEmpty())
			throw new java.lang.Exception ("DiscountCurve de-serializer: Empty state");

		java.lang.String strSerializedDiscountCurve = strRawString.substring (0, strRawString.indexOf
			(getObjectTrailer()));

		if (null == strSerializedDiscountCurve || strSerializedDiscountCurve.isEmpty())
			throw new java.lang.Exception ("DiscountCurve de-serializer: Cannot locate state");

		java.lang.String[] astrField = org.drip.analytics.support.GenericUtil.Split
			(strSerializedDiscountCurve, getFieldDelimiter());

		if (null == astrField || 4 > astrField.length)
			throw new java.lang.Exception ("DiscountCurve de-serializer: Invalid reqd field set");

		// double dblVersion = new java.lang.Double (astrField[0]);

		if (null == astrField[1] || astrField[1].isEmpty() ||
			org.drip.service.stream.Serializer.NULL_SER_STRING.equals (astrField[1]))
			throw new java.lang.Exception ("DiscountCurve de-serializer: Cannot locate start state");

		_dblStartDate = new java.lang.Double (astrField[1]);

		if (null == astrField[2] || astrField[2].isEmpty() ||
			org.drip.service.stream.Serializer.NULL_SER_STRING.equals (astrField[2]))
			throw new java.lang.Exception ("DiscountCurve de-serializer: Cannot locate currency");
		...
		if (null == astrField[3] || astrField[3].isEmpty() ||
			...
			throw new java.lang.Exception ("DiscountCurve de-serializer: Cannot decode state");
		...
		_adblCDFNode = new double[lsdblDate.size()];

		_adblCDFRate = new double[lsdblRate.size()];

		for (int i = 0; i < _adblCDFNode.length; ++i) {
			_adblCDFNode[i] = lsdblDate.get (i);

			_adblCDFRate[i] = lsdblRate.get (i);
		}
	}
	...
	@Override public org.drip.analytics.date.JulianDate getNodeDate (
		final int iNode)
	{
		if (iNode >= _adblCDFNode.length) return null;

		try {
			return new org.drip.analytics.date.JulianDate (_adblCDFNode[iNode]);
	...
	@Override public CalibratedDiscountCurve createParallelShiftedCurve (
		final double dblShift)
	{
		if (java.lang.Double.isNaN (dblShift)) return null;
		...
		CalibratedDiscountCurve dc = null;

		try {
			dc = new CalibratedDiscountCurve (new org.drip.analytics.date.JulianDate (_dblStartDate),
				_strCurrency, _adblCDFNode, _adblCDFRate);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		...
		org.drip.analytics.calibration.ComponentCalibrator calibrator = null;
		double[] adblCalibQuoteShifted = new double[_adblCalibQuote.length];

		try {
			if (USE_NR_CALIBRATOR)
				calibrator = new org.drip.analytics.calibration.NewtonRaphsonCalibrator();
			else
				calibrator = new org.drip.analytics.calibration.BracketingCalibrator();

			for (int i = 0; i < _adblCalibQuote.length; ++i)
				calibrator.bootstrapInterestRate (dc, null, null, _aCalibInst[i], i, _valParam,
					_astrCalibMeasure[i], adblCalibQuoteShifted[i] = _adblCalibQuote[i] + dblShift,
						_mmFixing, _quotingParams, false);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
	...
	@Override public CalibratedDiscountCurve createParallelRateShiftedCurve (
		final double dblShift)
	{
		if (java.lang.Double.isNaN (dblShift)) return null;

		double[] adblCDFRate = new double[_adblCDFRate.length];

		for (int i = 0; i < adblCDFRate.length; ++i)
			adblCDFRate[i] = _adblCDFRate[i] + dblShift;

		try {
			return new CalibratedDiscountCurve (new org.drip.analytics.date.JulianDate (_dblStartDate),
				_strCurrency, _adblCDFNode, adblCDFRate);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
	...
	@Override public CalibratedDiscountCurve createBasisRateShiftedCurve (
		final double[] adblDate,
		...
		final org.drip.param.definition.NodeTweakParams ntp)
	{
		...
		double[] adblCDFBumped = org.drip.analytics.support.AnalyticsHelper.BumpNTPNode (_adblCDFRate,
			ntp);
		...
		try {
			return new CalibratedDiscountCurve (new org.drip.analytics.date.JulianDate (_dblStartDate),
				_strCurrency, _adblCDFNode, adblCDFBumped);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
	...
	@Override public double getDF (
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblDate)) throw new java.lang.Exception ("DC.getDF got NaN for date");
		...
		while (i < _adblCDFRate.length && (int) dblDate >= (int) _adblCDFNode[i]) {
			dblExpArg -= _adblCDFRate[i] * (_adblCDFNode[i] - dblStartDate);
			dblStartDate = _adblCDFNode[i++];
		}

		if (i >= _adblCDFRate.length) i = _adblCDFRate.length - 1;

		dblExpArg -= _adblCDFRate[i] * (dblDate - dblStartDate);
	...
	@Override public double getDF (
		...
		throws java.lang.Exception
	{
		if (null == dt) throw new java.lang.Exception ("DC.getDF got null for date");
		...
		throws java.lang.Exception
	{
		if (null == strTenor || strTenor.isEmpty())
			throw new java.lang.Exception ("DC.getDF got bad tenor");

		return getDF (new org.drip.analytics.date.JulianDate (_dblStartDate).addTenor (strTenor));
	}
		...
		throws java.lang.Exception
	{
		if (null == dt1 || null == dt2)
			throw new java.lang.Exception ("DC.getEffectiveDF got null for date");
		...
		throws java.lang.Exception
	{
		if (null == strTenor1 || strTenor1.isEmpty() || null == strTenor2 || strTenor2.isEmpty())
			throw new java.lang.Exception ("DC.getEffectiveDF got bad tenor");
	...
	{
		if (java.lang.Double.isNaN (dblValue) || iNodeIndex > _adblCDFRate.length) return false;

		for (int i = iNodeIndex; i < _adblCDFRate.length; ++i)
			_adblCDFRate[i] = dblValue;
	...
	@Override public boolean setFlatValue (
		final double dblValue)
	{
		if (java.lang.Double.isNaN (dblValue)) return false;
	...
	@Override public double calcImpliedRate (
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblDt1) || java.lang.Double.isNaN (dblDt2))
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblDate))
			throw new java.lang.Exception ("DC.calcImpliedRate got NaN for date");
	...
	@Override public byte[] serialize()
	{
		...
		if (null == _adblCDFRate || 0 == _adblCDFRate.length)
			sb.append (org.drip.service.stream.Serializer.NULL_SER_STRING);
		else {
			for (int i = 0; i < _adblCDFRate.length; ++i) {
				if (0 != i) sb.append (getCollectionRecordDelimiter());

				sb.append (_adblCDFNode[i] + getCollectionKeyValueDelimiter() + _adblCDFRate[i]);
			}
		}
	...
	@Override public org.drip.service.stream.Serializer deserialize (
		final byte[] ab) {
		try {
			return new CalibratedDiscountCurve (ab);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
	...
	@Override public java.lang.String displayString()
	{
		try {
			...
			for (int i = 0; i < _adblCDFRate.length; ++i) {
				...
				sb.append (new org.drip.analytics.date.JulianDate (_adblCDFNode[i]) + "=" + _adblCDFRate[i]);
			}
		...
		throws java.lang.Exception
	{
		...
		CalibratedDiscountCurve dc = new CalibratedDiscountCurve
		...
		CalibratedDiscountCurve dcDeser = (CalibratedDiscountCurve) dc.deserialize (abDC);
