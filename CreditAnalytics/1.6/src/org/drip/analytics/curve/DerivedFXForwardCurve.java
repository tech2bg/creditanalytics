...
public class DerivedFXForwardCurve extends org.drip.analytics.definition.FXForwardCurve {
	...
	public DerivedFXForwardCurve (
		...
		throws java.lang.Exception
	{
		if (null == cp || null == dtSpot || java.lang.Double.isNaN (dblFXSpot) || null == adblDate || 0 ==
			adblDate.length || null == adblFXFwd || 0 == adblFXFwd.length || null == abIsPIP || 0 ==
				abIsPIP.length || adblDate.length != adblFXFwd.length || adblDate.length != abIsPIP.length)
		...
		for (int i = 0; i < abIsPIP.length; ++i) {
			if (java.lang.Double.isNaN (adblDate[i]) || adblDate[i] <= _dblSpotDate)
		...
		final double dblShift)
	{
		...
		try {
			return new DerivedFXForwardCurve (_cp, new org.drip.analytics.date.JulianDate (_dblSpotDate),
	...
	@Override public org.drip.service.stream.Serializer deserialize (
		final byte[] ab) {
		try {
			return new DerivedFXForwardCurve (ab);
		} catch (java.lang.Exception e) {
			e.printStackTrace();
		}
		...
		throws java.lang.Exception
	{
		...
		DerivedFXForwardCurve fxCurve = new DerivedFXForwardCurve (new org.drip.product.params.CurrencyPair
		...
		DerivedFXForwardCurve fxCurveDeser = new DerivedFXForwardCurve (abFXCurve);
