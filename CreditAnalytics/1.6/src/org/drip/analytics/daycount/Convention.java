...
public class Convention {
	...
	private static final boolean isLocSpecificHoliday (
		...
		final int iHolType)
	{
		if (null == strCalendarSet || strCalendarSet.isEmpty() || java.lang.Double.isNaN (dblDate))
			return false;
	...
	{
		if (null == strDayCount || strDayCount.isEmpty() || java.lang.Double.isNaN (dblStart) ||
			java.lang.Double.isNaN (dblEnd))
			throw new java.lang.Exception ("DayCount not set!");

		java.lang.String strDC = strDayCount.trim();

		if ("30/365".equalsIgnoreCase (strDC) || "ISMA 30/365".equalsIgnoreCase (strDC) ||
			"ISDA SWAPS:30/365".equalsIgnoreCase (strDC) || "ISDA30/365".equalsIgnoreCase (strDC) ||
				"ISDA 30E/365".equalsIgnoreCase (strDC)) {
			int iD1Adj = 0;
			int iD2Adj = 0;
			...
			if (bApplyEOMAdj) {
				if (org.drip.analytics.date.JulianDate.FEBRUARY == org.drip.analytics.date.JulianDate.Month
					...
					iD2Adj = (28 == org.drip.analytics.date.JulianDate.DaysInMonth
						(org.drip.analytics.date.JulianDate.Month (dblEnd),
							org.drip.analytics.date.JulianDate.Year (dblEnd)) ? 2 : 1);

				if (org.drip.analytics.date.JulianDate.FEBRUARY == org.drip.analytics.date.JulianDate.Month
					(dblStart) && org.drip.analytics.date.JulianDate.IsEOM (dblStart))
					iD1Adj = (28 == org.drip.analytics.date.JulianDate.DaysInMonth
						(org.drip.analytics.date.JulianDate.Month (dblStart),
							org.drip.analytics.date.JulianDate.Year (dblStart)) ? 2 : 1);

				if (31 == org.drip.analytics.date.JulianDate.Day (dblEnd) + iD2Adj && (30 ==
					org.drip.analytics.date.JulianDate.Day (dblStart) + iD1Adj || 31 ==
						org.drip.analytics.date.JulianDate.Day (dblStart) + iD1Adj))
					iD2Adj -= 1;

				if (31 == org.drip.analytics.date.JulianDate.Day (dblStart) + iD1Adj) iD1Adj -= 1;
			}

			return (365.* (org.drip.analytics.date.JulianDate.Year (dblEnd) -
				org.drip.analytics.date.JulianDate.Year (dblStart)) + 30. *
					(org.drip.analytics.date.JulianDate.Month (dblEnd) -
						org.drip.analytics.date.JulianDate.Month (dblStart) + iD2Adj - iD1Adj) +
							(org.drip.analytics.date.JulianDate.Day (dblEnd) -
								org.drip.analytics.date.JulianDate.Day (dblStart))) / 365.;
		}

		if ("30/360".equalsIgnoreCase (strDC) || "30U/360".equalsIgnoreCase (strDC) ||
			"360/360".equalsIgnoreCase (strDC) || "Bond basis".equalsIgnoreCase (strDC) ||
				"30/360 US".equalsIgnoreCase (strDC) || "US MUNI: 30/360".equalsIgnoreCase (strDC) ||
					"MUNI30/360".equalsIgnoreCase (strDC) || "ISDA30/360".equalsIgnoreCase (strDC)) {
			int iD1Adj = 0;
			int iD2Adj = 0;
			...
			return (360.* (org.drip.analytics.date.JulianDate.Year (dblEnd) -
				org.drip.analytics.date.JulianDate.Year (dblStart)) + 30. *
					(org.drip.analytics.date.JulianDate.Month (dblEnd) -
						org.drip.analytics.date.JulianDate.Month (dblStart)) +
							(org.drip.analytics.date.JulianDate.Day (dblEnd) -
								org.drip.analytics.date.JulianDate.Day (dblStart) + iD2Adj - iD1Adj)) / 360.;
		}
		...
		if (bApplyEOMAdj) {
			if (org.drip.analytics.date.JulianDate.IsEOM (dblStart))
				iD1Adj = 30 - org.drip.analytics.date.JulianDate.Day (dblStart);
			...
			if (org.drip.analytics.date.JulianDate.IsEOM (dblEnd) &&
				...
				iD2Adj = 30 - org.drip.analytics.date.JulianDate.Day (dblEnd);
		}

		if ("30E/360".equalsIgnoreCase (strDC) || "30/360 ICMA".equalsIgnoreCase (strDC) ||
			"30S/360".equalsIgnoreCase (strDC) || "Eurobond basis".equalsIgnoreCase (strDC) ||
				"Eurobond basis (ISDA 2006)".equalsIgnoreCase (strDC) || "Special German".equalsIgnoreCase
					(strDC) || "ISMA 30/360".equalsIgnoreCase (strDC) || "30E/360 ISDA".equalsIgnoreCase
						(strDC) || "Eurobond basis (ISDA 2000)".equalsIgnoreCase (strDC) ||
							"German".equalsIgnoreCase (strDC) || "German:30/360".equalsIgnoreCase (strDC) ||
								"Ger:30/360".equalsIgnoreCase (strDC) || "ISDA SWAPS:30/360".equalsIgnoreCase
									(strDC) || "ISDA 30E/360".equalsIgnoreCase (strDC))
			return (360.* (org.drip.analytics.date.JulianDate.Year (dblEnd) -
				org.drip.analytics.date.JulianDate.Year (dblStart)) + 30. *
					(org.drip.analytics.date.JulianDate.Month (dblEnd) -
						org.drip.analytics.date.JulianDate.Month (dblStart)) +
		...
		if (("Actual/Actual".equalsIgnoreCase (strDC) || "Actual/Actual ICMA".equalsIgnoreCase (strDC) ||
			"Act/Act ICMA".equalsIgnoreCase (strDC) || "ISMA-99".equalsIgnoreCase (strDC) ||
				"Act/Act ISMA".equalsIgnoreCase (strDC)) && null != actactParams)
			return (dblEnd - dblStart + iD2Adj - iD1Adj) / actactParams._iFreq / (actactParams._dblEnd -
				actactParams._dblStart);

		if (("ISMA 30/Act".equalsIgnoreCase (strDC) || "30/Act".equalsIgnoreCase (strDC) ||
			"ISMA 30/Act".equalsIgnoreCase (strDC) || "ISDA SWAPS:30/Act".equalsIgnoreCase (strDC) ||
				"ISDA30/Act".equalsIgnoreCase (strDC) || "ISDA 30E/ACT".equalsIgnoreCase (strDC)) && null !=
					actactParams)
			return org.drip.analytics.date.JulianDate.Year (dblEnd) - org.drip.analytics.date.JulianDate.Year
				(dblStart) + (30. * (org.drip.analytics.date.JulianDate.Month (dblEnd) -
					org.drip.analytics.date.JulianDate.Month (dblStart)) +
						(org.drip.analytics.date.JulianDate.Day (dblEnd) -
							org.drip.analytics.date.JulianDate.Day (dblStart) + iD2Adj - iD1Adj)) /
								actactParams._iFreq / (actactParams._dblEnd - actactParams._dblStart);

		if (("Actual/Actual ISDA".equalsIgnoreCase (strDC) || "Actual/Actual".equalsIgnoreCase (strDC) ||
			"Act/Act".equalsIgnoreCase (strDC) || "Actual/365".equalsIgnoreCase (strDC) ||
				"US:WIT Act/Act".equalsIgnoreCase (strDC)))
			return ((org.drip.analytics.date.JulianDate.DaysRemaining (dblStart) - iD1Adj) /
				(org.drip.analytics.date.JulianDate.IsLeapYear (dblStart) ? 366. : 365.)) +
					((org.drip.analytics.date.JulianDate.DaysElapsed (dblEnd) + iD2Adj) /
		...
		if (("Actual/365 Fixed".equalsIgnoreCase (strDC) || "Act/365 Fixed".equalsIgnoreCase (strDC) ||
			"A/365 Fixed".equalsIgnoreCase (strDC) || "A/365F".equalsIgnoreCase (strDC) ||
				"Act/365F".equalsIgnoreCase (strDC) || "English".equalsIgnoreCase (strDC) ||
					"Act/365".equalsIgnoreCase (strDC)))
			return (dblEnd - dblStart + iD2Adj - iD1Adj) / 365.;

		if ("Actual/365 JGB".equalsIgnoreCase (strDC) || "Actual/365 JGB (NL)".equalsIgnoreCase (strDC) ||
			"NL/365".equalsIgnoreCase (strDC))
			return (dblEnd - dblStart - org.drip.analytics.date.JulianDate.NumFeb29 (dblStart, dblEnd,
				org.drip.analytics.date.JulianDate.RIGHT_INCLUDE) + iD2Adj - iD1Adj) / 365.;

		if (("Actual/360 Fixed".equalsIgnoreCase (strDC) || "Act/360".equalsIgnoreCase (strDC) ||
			"A/360".equalsIgnoreCase (strDC) || "French".equalsIgnoreCase (strDC) ||
				"US:WIB Act/360".equalsIgnoreCase (strDC)))
			return (dblEnd - dblStart + iD2Adj - iD1Adj) / 360.;

		if (("Actual/365L".equalsIgnoreCase (strDC) || "ISMA-Year".equalsIgnoreCase (strDC) ||
			"Actual/Actual AFB".equalsIgnoreCase (strDC)) && null != actactParams) {
			if (1 == actactParams._iFreq) {
				if (org.drip.analytics.date.JulianDate.ContainsFeb29 (dblStart, dblEnd,
					...
					return (dblEnd - dblStart + iD2Adj - iD1Adj) / 366.;
		...
		if ("NL/360".equalsIgnoreCase (strDC))
			return (dblEnd - dblStart + iD2Adj - iD1Adj - org.drip.analytics.date.JulianDate.NumFeb29
				(dblStart, dblEnd, org.drip.analytics.date.JulianDate.RIGHT_INCLUDE)) / 360.;

		if ("NL/Act".equalsIgnoreCase (strDC) && null != actactParams)
			return (dblEnd - dblStart + iD2Adj - iD1Adj - org.drip.analytics.date.JulianDate.NumFeb29
		...
		if ("Act/364".equalsIgnoreCase (strDC))
			return (org.drip.analytics.date.JulianDate.DaysRemaining (dblStart) /
				(org.drip.analytics.date.JulianDate.IsLeapYear (dblStart) ? 366. : 364.)) +
					(org.drip.analytics.date.JulianDate.DaysElapsed (dblEnd) /
						(org.drip.analytics.date.JulianDate.IsLeapYear (dblEnd) ? 366. : 364.) + iD2Adj -
							iD1Adj) + org.drip.analytics.date.JulianDate.Year (dblEnd) -
								org.drip.analytics.date.JulianDate.Year (dblStart) - 1;

		if ("BUS252".equalsIgnoreCase (strDC) || "BUS DAYS252".equalsIgnoreCase (strDC) ||
			"BUS/252".equalsIgnoreCase (strDC))
			return BusDays (dblStart, dblEnd, strCalendar) / 252.;

		System.out.println ("Unknown DC: " + strDC + "; defaulting to Actual/365.25");

		return (dblEnd - dblStart + iD2Adj - iD1Adj) / 365.25;
	}
	...
	public static final double RollDate (
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblDate)) throw new java.lang.Exception ("Cannot roll a NaN date");
	...
	public static final int BusDays (
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblStart) || java.lang.Double.isNaN (dblFinish))
	...
	public static final java.util.List<java.lang.Double> HolidaySet (
		...
		final java.lang.String strCalendar)
	{
		if (java.lang.Double.isNaN (dblStart) || java.lang.Double.isNaN (dblFinish)) return null;
	...
	public static final double Adjust (
		...
		throws java.lang.Exception
	{
		if (java.lang.Double.isNaN (dblDate)) throw new java.lang.Exception ("Cannot adjust a NaN date!");
