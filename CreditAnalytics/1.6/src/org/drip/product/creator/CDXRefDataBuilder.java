...
public class CDXRefDataBuilder {
	/**
	 * Index Curve ID
	 */
	...
	public static final CDXRefDataBuilder CreateCDXRefDataBuilder (
	...
	{
		CDXRefDataBuilder cdxrd = new CDXRefDataBuilder();
	...
	public boolean setCoupon (
		final double dblCoupon)
	{
		if (java.lang.Double.isNaN (_dblCoupon = dblCoupon)) return false;
	...
	public boolean setRecovery (
		final double dblRecovery)
	{
		if (java.lang.Double.isNaN (_dblRecovery = dblRecovery)) return false;
	...
	public boolean setIndexFactor (
		final double dblIndexFactor)
	{
		if (java.lang.Double.isNaN (_dblIndexFactor = dblIndexFactor)) return false;
		...
		if (null == _strCurveID || _strCurveID.isEmpty() || null == _strSPN || _strSPN.isEmpty() || null ==
			_strIndexLabel || _strIndexLabel.isEmpty() || null == _strIndexName || _strIndexName.isEmpty() ||
				null == _strCurveName || _strCurveName.isEmpty() || null == _dtIssue || null == _dtMaturity
					|| _dtIssue.getJulian() >= _dtMaturity.getJulian() || java.lang.Double.isNaN (_dblCoupon)
						|| null == _strCurrency || _strCurrency.isEmpty() || null == _strDayCount ||
							_strDayCount.isEmpty() || java.lang.Double.isNaN (_dblRecovery) || null ==
								_strRedID || _strRedID.isEmpty() || null == _strIndexClass ||
	...
	/**
	 * Creates a CDX Product using the reference data contained herein.
	 * 
	 * @return BasketCDX Product
	 */

	public org.drip.product.definition.BasketProduct ConstructCDX()
	{
		java.lang.String[] astrCC = new java.lang.String[_iOriginalComponentCount];

		for (int i = 0; i < _iOriginalComponentCount; ++i)
			astrCC[i] = "CC" + (i + 1);

		return org.drip.product.creator.CDSBasketBuilder.MakeCDX (_dtMaturity.subtractTenor (_iIndexLifeSpan
			+ "Y"), _dtMaturity, _dblCoupon, _strCurrency, astrCC, _strIndexClass + "." + _strIndexGroupName
				+ "." + _iIndexLifeSpan + "Y." + _iIndexSeries + "." + _iIndexVersion);
	}
}
