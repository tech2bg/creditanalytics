...
public class CDSComponent extends org.drip.product.definition.CreditDefaultSwap {
	...
	public CDSComponent (
		...
		throws java.lang.Exception
	{
		if (null == strIR || strIR.isEmpty() || null == crValParams || java.lang.Double.isNaN (dblEffective)
			|| java.lang.Double.isNaN (dblMaturity) || java.lang.Double.isNaN (dblCoupon))
	...
	@Override public double getNotional (
		...
		throws java.lang.Exception
	{
		if (null == _notlSchedule || java.lang.Double.isNaN (dblDate))
	...
	@Override public double getNotional (
		...
		throws java.lang.Exception
	{
		if (null == _notlSchedule || java.lang.Double.isNaN (dblDate1) || java.lang.Double.isNaN (dblDate2))
			throw new java.lang.Exception ("Bad date into getNotional");
		...
		throws java.lang.Exception
	{
		...
		if (java.lang.Double.isNaN (_dblCoupon = dblCoupon))
		...
		final org.drip.param.valuation.QuotingParams quotingParams)
	{
		...
		java.lang.String strMarketMeasure = "";
		...
		if (!java.lang.Double.isNaN (dblCreditBasis)) {
	...
	public double calibFlatSpread (
		...
		throws java.lang.Exception
	{
		SpreadCalibOP scop = new SpreadCalibrator (this, "Upfront",
			SpreadCalibrator.CALIBRATION_TYPE_FLAT_CURVE_NODES).calibrateHazardFromPriceNR (valParams,
	...
	public class SpreadCalibOP {
		...
		public SpreadCalibOP (
			...
			throws java.lang.Exception
		{
			if (java.lang.Double.isNaN (_dblCalibResult = dblCalibResult) || null == (_ccCalib = ccCalib))
	...
	public class SpreadCalibrator {
		...
		// Spread Calibration Stochastic Control

		private double _dblSpreadInit = 0.01;
		private int _iNumSpreadIterations = 50;
		private double _dblSpreadIncr = 0.0001;
		private double _dblSpreadDiffTol = 0.0001;
		...
		public SpreadCalibrator (
			...
			throws java.lang.Exception
		{
			if (null == (_cds = cds) || null == (_strCalibMeasure = strCalibMeasure) ||
				strCalibMeasure.isEmpty() || (CALIBRATION_TYPE_FLAT_INSTRUMENT_NODE != (_iCalibType =
					iCalibType) && CALIBRATION_TYPE_FLAT_CURVE_NODES != iCalibType &&
						CALIBRATION_TYPE_NODE_PARALLEL_BUMP != iCalibType))
		...
		public SpreadCalibOP calibrateHazardFromPriceNR (
			final org.drip.param.valuation.ValuationParams valParams,
			final org.drip.param.pricer.PricerParams pricerParams,
			final org.drip.param.definition.ComponentMarketParams mktParams,
			final org.drip.param.valuation.QuotingParams quotingParams,
		...
		{
			if (null == valParams || null == pricerParams || null == mktParams  || null ==
				mktParams.getCreditCurve() || java.lang.Double.isNaN (dblPriceCalib))
				throw new java.lang.Exception ("calibrateHazardFromPriceNR - bad inputs");

			double dblSpreadPrev = _dblSpreadInit;

			org.drip.analytics.definition.CreditCurve ccOld = mktParams.getCreditCurve();

			if (CALIBRATION_TYPE_NODE_PARALLEL_BUMP != _iCalibType)
				mktParams.setCreditCurve (ccOld.createFlatCurve (dblSpreadPrev,
				...
				mktParams.setCreditCurve ((org.drip.analytics.definition.CreditCurve)
					ccOld.createTweakedCurve (new org.drip.param.definition.NodeTweakParams
						(org.drip.param.definition.NodeTweakParams.NODE_FLAT_TWEAK, false, dblSpreadPrev)));

			double dblPrice = _cds.calcMeasureValue (valParams, pricerParams, mktParams, quotingParams,
				"Upfront");

			if (CALIBRATION_TYPE_NODE_PARALLEL_BUMP != _iCalibType)
				mktParams.setCreditCurve (ccOld.createFlatCurve (dblSpreadPrev + _dblSpreadIncr,
					CALIBRATION_TYPE_FLAT_CURVE_NODES == _iCalibType, java.lang.Double.NaN));
			else
				mktParams.setCreditCurve ((org.drip.analytics.definition.CreditCurve)
					ccOld.createTweakedCurve (new org.drip.param.definition.NodeTweakParams
			...
			double dblDSpreadDP = _dblSpreadIncr / (_cds.calcMeasureValue (valParams, pricerParams,
				mktParams, quotingParams, _strCalibMeasure) - dblPrice);

			double dblSpread = dblSpreadPrev + dblDSpreadDP * (dblPriceCalib - dblPrice);

			if (java.lang.Double.isNaN (dblSpread) || java.lang.Double.NEGATIVE_INFINITY == dblSpread ||
				java.lang.Double.POSITIVE_INFINITY == dblSpread)
				throw new java.lang.Exception ("Got " + dblSpread + " for Spread for " +
					_cds.getComponentName() + " and price " + dblPrice);

			while (_dblSpreadDiffTol < java.lang.Math.abs (dblSpread - dblSpreadPrev)) {
				if (0 == --_iNumSpreadIterations)
					throw new java.lang.Exception ("Cannot calib Spread for " + _cds.getComponentName() +
						" and price " + dblPriceCalib + " within limit!");

				if (CALIBRATION_TYPE_NODE_PARALLEL_BUMP != _iCalibType)
					mktParams.setCreditCurve (ccOld.createFlatCurve (dblSpreadPrev = dblSpread,
						CALIBRATION_TYPE_FLAT_CURVE_NODES == _iCalibType, java.lang.Double.NaN));
				else
					mktParams.setCreditCurve ((org.drip.analytics.definition.CreditCurve)
						ccOld.createTweakedCurve (new org.drip.param.definition.NodeTweakParams
				...
				dblPrice = _cds.calcMeasureValue (valParams, pricerParams, mktParams, quotingParams,
					_strCalibMeasure);
				...
				dblDSpreadDP = _dblSpreadIncr / (_cds.calcMeasureValue (valParams, pricerParams, mktParams,
					quotingParams, _strCalibMeasure) - dblPrice);

				dblSpread = dblSpreadPrev + dblDSpreadDP * (dblPriceCalib - dblPrice);
			...
			try {
				if (mktParams.setCreditCurve (ccOld)) return new SpreadCalibOP (dblSpread, ccOld);
			} catch (java.lang.Exception e) {
				e.printStackTrace();
			}
