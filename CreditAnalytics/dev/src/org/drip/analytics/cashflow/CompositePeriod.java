...
public abstract class CompositePeriod {
		...
		final org.drip.param.market.CurveSurfaceQuoteSet csqs)
	{
		...
		org.drip.function.deterministic.R1ToR1 auCreditVol = csqs.creditCurveVolSurface (creditLabel);

		org.drip.function.deterministic.R1ToR1 auForwardVol = csqs.forwardCurveVolSurface
			(forwardLabel);

		org.drip.function.deterministic.R1ToR1 auFundingVol = csqs.fundingCurveVolSurface
			(fundingLabel);

		org.drip.function.deterministic.R1ToR1 auFXVol = csqs.fxCurveVolSurface (fxLabel);

		org.drip.function.deterministic.R1ToR1 auCreditForwardCorr = csqs.creditForwardCorrSurface
			(creditLabel, forwardLabel);

		org.drip.function.deterministic.R1ToR1 auForwardFundingCorr = csqs.forwardFundingCorrSurface
			(forwardLabel, fundingLabel);

		org.drip.function.deterministic.R1ToR1 auForwardFXCorr = csqs.forwardFXCorrSurface
