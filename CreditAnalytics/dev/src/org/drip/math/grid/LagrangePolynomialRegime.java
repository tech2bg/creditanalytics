...
public class LagrangePolynomialRegime implements org.drip.math.grid.SingleSegmentRegime {
	...
	@Override public boolean setup (
		...
		final org.drip.math.grid.RegimeCalibrationSetting ss)
	{
		...
		for (int i = 0; i < iNumPredictorOrdinate; ++i) {
			...
			try {
				...
				if (!lps.setup (adblSensitivityShiftedInputResponse[0], adblSensitivityShiftedInputResponse,
					new org.drip.math.grid.RegimeCalibrationSetting
						(org.drip.math.grid.RegimeCalibrationSetting.BOUNDARY_CONDITION_FLOATING,
							org.drip.math.grid.RegimeCalibrationSetting.CALIBRATE)) ||
								!wjDResponseDResponseInput.accumulatePartialFirstDerivative (0, i,
									(lps.response (dblPredictorOrdinate) -
