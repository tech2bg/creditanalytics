...
public class MultiSegmentCalibratableRegime extends org.drip.math.function.AbstractUnivariate implements
		...
		final org.drip.math.regime.RegimeCalibrationSetting rcs)
	{
		...
		if (org.drip.math.regime.RegimeCalibrationSetting.BOUNDARY_CONDITION_FLOATING ==
			rcs.getCalibrationBoundaryCondition()) {
			if (!initLeftMostSegment (0.) || !calibSegment (1, aRVC)) return false;
		} else if (0 != (org.drip.math.regime.RegimeCalibrationSetting.CALIBRATE & rcs.getCalibrationDetail()))
		{
		...
		if (0 != (org.drip.math.regime.RegimeCalibrationSetting.CALIBRATE_JACOBIAN &
		...
		final int iSetupMode)
	{
		...
		for (int i = 0; i < iNumSegment; ++i) {
			try {
				if (0 != (org.drip.math.regime.RegimeCalibrationSetting.CALIBRATE & iSetupMode) &&
		...
		if (0 != (org.drip.math.regime.RegimeCalibrationSetting.CALIBRATE_JACOBIAN & iSetupMode)) {
			try {
				if (null == (_wjDCoeffDEdgeParams = new org.drip.math.calculus.WengertJacobian
					(_aSegment[0].numBasis(), iNumSegment + 1)))
					return false;
	...
	@Override public double evaluate (
		...
		throws java.lang.Exception
	{
		if (org.drip.math.regime.RegimeCalibrationSetting.BOUNDARY_CONDITION_NATURAL ==
		...
		if (org.drip.math.regime.RegimeCalibrationSetting.BOUNDARY_CONDITION_FINANCIAL ==
			_tep._iCalibrationBoundaryCondition) {
			if (!initLeftMostSegment (dblLeftSlope) || !calibSegment (1, _tep._aRVC))
				throw new java.lang.Exception
