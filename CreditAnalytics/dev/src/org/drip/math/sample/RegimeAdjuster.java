...
public class RegimeAdjuster {
		...
		throws Exception
	{
		...
		PredictorResponseBuilderParams prbp = PolynomialSegmentControlParams (iPolyNumBasis, segParams, rsc);
		...
		MultiSegmentRegime regimeLeftClipped = regimeBase.clipLeft ("LEFT_CLIP", 1.75);
		...
		MultiSegmentRegime regimeRightClipped = regimeBase.clipRight ("RIGHT_CLIP", 8.75);
		...
		while (dblX <= dblXMax) {
			...
			java.lang.String strDisplay = "Y[" + FormatUtil.FormatDouble (dblX, 2, 2, 1.) + "] => "
				+ FormatUtil.FormatDouble (regimeBase.response (dblX), 2, 2, 1.) + " | "
				+ regimeBase.monotoneType (dblX);

			if (regimeLeftClipped.in (dblX)) {
				strLeftClippedValue = FormatUtil.FormatDouble (regimeLeftClipped.response (dblX), 2, 2, 1.);

				strLeftClippedMonotonocity = regimeLeftClipped.monotoneType (dblX).toString();
			}

			if (regimeRightClipped.in (dblX)) {
				strRightClippedValue = FormatUtil.FormatDouble (regimeRightClipped.response (dblX), 2, 2, 1.);
