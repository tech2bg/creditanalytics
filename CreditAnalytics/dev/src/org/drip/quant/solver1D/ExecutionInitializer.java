...
public class ExecutionInitializer {
	...
	private StartingVariateOF validateVariate (
		final double dblVariate,
		final org.drip.quant.solver1D.BracketingOutput bop)
	{
		...
		final org.drip.quant.solver1D.BracketingOutput bop)
	{
			...
			if (org.drip.quant.common.NumberUtil.IsValid (dblVariatePrev) &&
				...
				try {
					bop.done (dblVariatePrev, dblVariate, dblOFPrev, dblOF,
						org.drip.quant.solver1D.VariateIteratorPrimitive.Bisection (dblVariatePrev,
							dblVariate));
				} catch (java.lang.Exception e) {
				}
		...
		try {
			bop.done (dblVariateLeft, dblVariateRight, dblOFLeft, dblOFRight,
				org.drip.quant.solver1D.VariateIteratorPrimitive.Bisection (dblVariateLeft, dblVariateRight));
		} catch (java.lang.Exception e) {
		}
	...
	private boolean isInConvergenceZone (
		...
		final double dblOFGoal,
		final org.drip.quant.solver1D.ConvergenceOutput cop)
		throws java.lang.Exception
	{
		if (!cop.incrOFCalcs())
	...
	private double getStartingBracketVariate (
		final org.drip.quant.solver1D.BracketingControlParams bcp,
		final org.drip.quant.solver1D.InitializationHeuristics ih)
	{
	...
	public org.drip.quant.solver1D.BracketingOutput initializeBracket (
		final org.drip.quant.solver1D.InitializationHeuristics ih,
		final double dblOFGoal)
	{
		org.drip.quant.solver1D.BracketingControlParams bcp = (null != ih && null != ih.getCustomBCP()) ?
			ih.getCustomBCP() : new org.drip.quant.solver1D.BracketingControlParams();

		int iNumExpansions = bcp.getNumExpansions();

		org.drip.quant.solver1D.BracketingOutput bop = new org.drip.quant.solver1D.BracketingOutput();
	...
	public org.drip.quant.solver1D.ConvergenceOutput initializeVariate (
		final org.drip.quant.solver1D.InitializationHeuristics ih,
		final double dblOFGoal)
	{
		if (!org.drip.quant.common.NumberUtil.IsValid (dblOFGoal)) return null;

		org.drip.quant.solver1D.ConvergenceOutput cop = new org.drip.quant.solver1D.ConvergenceOutput();

		org.drip.quant.solver1D.BracketingOutput bop = initializeBracket (ih, dblOFGoal);
	...
	public org.drip.quant.solver1D.BracketingOutput verifyHardSearchEdges (
		final org.drip.quant.solver1D.InitializationHeuristics ih,
		final double dblOFGoal)
	{
