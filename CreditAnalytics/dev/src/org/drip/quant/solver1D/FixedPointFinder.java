...
public abstract class FixedPointFinder {
		...
		throws java.lang.Exception
	{
		...
		_ec = new org.drip.quant.solver1D.ExecutionControl (of, null);

		_bWhine = bWhine;
	}

	protected abstract boolean iterateVariate (
		final org.drip.quant.solver1D.IteratedVariate vi,
		final org.drip.quant.solver1D.FixedPointFinderOutput rfop);

	protected abstract org.drip.quant.solver1D.ExecutionInitializationOutput initializeVariateZone (
	...
	public org.drip.quant.solver1D.FixedPointFinderOutput findRoot (
		final org.drip.quant.solver1D.InitializationHeuristics ih)
	{
		org.drip.quant.solver1D.FixedPointFinderOutput rfop = null;

		org.drip.quant.solver1D.ExecutionInitializationOutput eiop = initializeVariateZone (ih);

		if (null == eiop || !eiop.isDone()) return null;

		try {
			rfop = new org.drip.quant.solver1D.FixedPointFinderOutput (eiop);
			...
			org.drip.quant.solver1D.IteratedVariate iv = new org.drip.quant.solver1D.IteratedVariate (eiop,
				dblOF);
