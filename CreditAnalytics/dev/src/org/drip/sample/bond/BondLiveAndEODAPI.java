...
public class BondLiveAndEODAPI {
		...
		throws Exception
	{
		...
		for (String strISIN : lsstrISIN) {
			...
			if (null != bond && !bond.hasVariableCoupon() && !bond.hasBeenExercised() && !bond.hasDefaulted()
				...
				System.out.println (strISIN + FIELD_SEPARATOR +
					...
					FormatUtil.FormatDouble (bond.coupon (dtToday.julian(), null, mktParams).compoundedAccrualRate(), 2, 3, 100.) + FIELD_SEPARATOR +
		...
		for (String strISIN : lsstrISIN) {
			...
			System.out.println (
				...
				FormatUtil.FormatDouble (bond.coupon (JulianDate.Today().julian(), null, null).compoundedAccrualRate(), 2, 3, 100.) + FIELD_SEPARATOR +
				bond.maturity() + FIELD_SEPARATOR +
		...
		throws Exception
	{
		...
		System.out.println (strISIN + "    " + bond.getTicker() + " " + FormatUtil.FormatDouble (bond.coupon
			(valParams.valueDate(), valParams, mktParams).compoundedAccrualRate(), 2, 3, 100.) + " " + bond.maturity());
