...
public class BondManager {
			...
			final double dblAskPrice)
	{
		...
		if (null != bond && bond.maturity().julian() > dt.julian()) {
			...
			try {
				java.lang.String strRunName = bond.getTicker() + "  " + df2p.format (100. * bond.coupon
					(valParams.valueDate(), valParams, null).nominalAccrualRate()) + " " +
		...
		final org.drip.analytics.date.JulianDate dt)
	{
		...
		for (java.util.Map.Entry<java.lang.String, org.drip.product.credit.BondComponent> me :
			...
			if (bond.maturity().julian() > dt.julian()) {
				...
				try {
					java.lang.String strRunName = strRunTicker + "  " + df2_3p.format (100. * bond.coupon
						(dt.julian(), null, null).nominalAccrualRate()) + " " +
							(org.drip.analytics.date.JulianDate.Year (bond.maturity().julian()) - 2000);
		...
		final double dblPrice)
	{
		...
		try {
			...
			dblASWSpread = bond.calcASWFromPrice (valParams, mpc.getScenMarketParams (bond, "Base"), null,
				wi.date(), wi.factor(), dblPrice);
